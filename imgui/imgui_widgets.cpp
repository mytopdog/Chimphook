...
bool ImGui::ToggleButton(const char* label, bool* v, const ImVec2& size_arg)
{
	...
	// Render
	const ImU32 col1 = ImGuiCol_Button;
	const ImU32 col2 = GetColorU32((held && hovered || *v) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
	RenderNavHighlight(bb, id);
	RenderFrame(bb.Min, bb.Max, col1, true, style.FrameRounding);
	if (held || hovered || *v) {
		RenderFrame(ImVec2(bb.Min.x, bb.Max.y), ImVec2(bb.Max.x, bb.Max.y + 2), col2, true, 0);
	}
	RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::BeginGroupBox(const char* label, const ImVec2& size_arg)
{
	...
	auto text_size = ImGui::CalcTextSize(label, NULL, true);
...
bool ImGui::Checkbox(const char* label, bool* v, bool ch)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
