...
std::vector<ImTextureID> Menu::Render(IDirect3DDevice9* pDevice)
{
		...
		ImGuiWindowFlags_NoNavFocus ))
	{
		...
		{
			static int mstabselected = 0;
			switch (tabSelected)
			{
			case 0:
				ImGui::BeginGroup();
				{
					bool vssel = mstabselected == 1;
					if (ImGui::ToggleButton("Misc Part 1", &vssel, ImVec2(ImGui::GetWindowSize().x / 4, 20)))
					{
						mstabselected = 1;
					}
					vssel = mstabselected == 0;
					ImGui::SameLine(0, 0);
					if (ImGui::ToggleButton("Misc Part 2", &vssel, ImVec2(ImGui::GetWindowSize().x / 4, 20)))
					{
						mstabselected = 0;
					}
					vssel = mstabselected == 3;
					ImGui::SameLine(0, 0);
					if (ImGui::ToggleButton("Misc Part 3", &vssel, ImVec2(ImGui::GetWindowSize().x / 4, 20)))
					{
						mstabselected = 3;
					}
				}
