...
public:
	...
	{
		yaw = minVal + ((float)rand() / RAND_MAX) * (maxVal - minVal);
		pitch = minVal + ((float)rand() / RAND_MAX) * (maxVal - minVal);
		roll = minVal + ((float)rand() / RAND_MAX) * (maxVal - minVal);
	}
